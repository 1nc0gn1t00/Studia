...
def render(time):
    ...
    # Ustawiamy kamerę (V_matrix) tak żeby objąć planszę 10x10
    V_matrix = glm.lookAt(
    ...
    # PĘTLA DO RYSOWANIA PLANSZY 10x10
    # Iterujemy od -5 do 5, aby plansza była wyśrodkowana
    for x in range(-5, 5):
        for y in range(-5, 5):
            # 1. Macierz jednostkowa
            M_matrix = glm.mat4(1.0)

            # 2. Translacja - przesuwamy sześcian na pozycję w siatce
            # Mnożymy x i y przez np. 1.0 lub 1.5, żeby zrobić odstępy
            M_matrix = glm.translate(M_matrix, glm.vec3(x * 1.0, y * 1.0, 0.0))

            # 3. Rotacja - obracamy sześcian wokół własnej osi
            M_matrix = glm.rotate(M_matrix, time, glm.vec3(1.0, 1.0, 0.0))

            # 4. Wysłanie macierzy M dla AKTUALNEGO sześcianu
            glUniformMatrix4fv(M_location, 1, GL_FALSE, glm.value_ptr(M_matrix))

            # 5. Rysowanie sześcianu
