...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void) {
            // Obliczenie pozycji w siatce 10x10 na podstawie numeru instancji (Zadanie 4.5)
            // gl_InstanceID to numer aktualnie rysowanej kopii (od 0 do 99)
            float x = gl_InstanceID % 10 - 5.0; 
            float y = gl_InstanceID / 10 - 5.0;
            vec4 offset = vec4(x, y, 0.0, 0.0);

            // Aplikujemy rotację (M_matrix) do wierzchołka, dodajemy przesunięcie (offset),
            // a na końcu rzutujemy kamerą (V i P).
...
def main():
    ...
    window = glfwCreateWindow(380, 380, __file__, None, None)
    if not window:
        glfwTerminate()
        sys.exit(-1)
